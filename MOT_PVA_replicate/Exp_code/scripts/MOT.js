...
function path_and_number_init(startPoints, trialNumber,jumps) {
    critical_range = range(2, jumps - 2, 1);
    critical_idx = randSample(critical_range, critical_time);
    console.log(critical_idx)
    objectBag = object_init(startPoints);
    critical_index = shuffled_trials[trialNumber];
    path_dat = init_path(critical_index, startPoints, objectBag, profile, path_length, jumps, critical_idx);
...
function init_path(critical_index, startPoints, objectBag, profile, path_length, jumps, critical_idx) {
    let path_dat;
    if (critical_index == 0) {
        path_dat = position_case(startPoints, objectBag, profile, path_length, jumps);
    } else if (critical_index == 1) {
        path_dat = velocity_case(startPoints, objectBag, profile, path_length, jumps, critical_idx);
    } else if (critical_index == 2) {
        path_dat = acceleration_case(startPoints, objectBag, profile, path_length, jumps, critical_idx);
    } else if (critical_index == 3) {
        path_dat = pos_only_case(startPoints, objectBag, profile, path_length, jumps, critical_idx);        
    }
    return path_dat;
}
...
document.querySelector("#btnStart").addEventListener('click', function() {
    if (btn.innerHTML == "Start"){
        init_info = path_and_number_init(startPoints, trialNumber, jumps);
