...
function rand_pos_only(pos_only) {
    index = Math.floor(Math.random() * 2);
    return pos_only[index];
}
...
nei_move_0 = [[0,0],[0,2],[1,0],[1,1],[1,3],[1,4],[1,5],[2,0],[3,1],[4,0],[5,0]];

nei_move_1 = [[1,1],[1,3],[0,1],[0,0],[0,2],[0,4],[0,5],[3,1],[2,0],[4,1],[5,1]]; 

nei_to_nei_0 = [[0,0],[0,2],[1,1],[1,3],[3,1]];

nei_to_nei_1 = [[1,1],[1,3],[0,0],[0,2],[2,0]];
...
//Simultaneity = 4
function velocity_case(startPoints, objectBag, profile, path_length, jumps, critical_idx) {
    let path_dat = [];
    let profile_index = [];
    let dir_index = [];
    let status = [];
    ...
    let tmp_dir_index;
    let tmp_path_dat = [];
    let before_critical = critical_idx - 1;
    for (let i = 0; i < jumps; i++) {
        if (i == 0) {
            profile_index = checkPos(objectBag, startPoints);
            status = checkStatus(profile_index);
            dir_index = RandDir_Pos(profile_index, status[i]);
            path_dat = object_path(profile, profile_index, dir_index);
            console.log(critical_idx - 1);
        } else {
            if (i == 1 && !(i == before_critical)) {
                objectUpdate = newStartPoints(profile, profile_index, dir_index, path_length);
                objectBag = combineArray(objectBag, objectUpdate);
                tmp_profile_index = checkPos(objectUpdate, startPoints)
                profile_index = combineArray(profile_index, tmp_profile_index);
                ...
                tmp_path_dat = object_path(profile, tmp_profile_index, tmp_dir_index);
                path_dat = concatArr(path_dat, tmp_path_dat);
            } else if (i == before_critical && !(i == critical_idx)) {
                objectUpdate = newStartPoints(profile, tmp_profile_index, tmp_dir_index, path_length);
                objectBag = addArray(objectBag, objectUpdate);
                tmp_profile_index = checkPos(objectUpdate, startPoints);
                profile_index = addArray(profile_index, tmp_profile_index);
...
//Simultaneity = 4
function acceleration_case(startPoints, objectBag, profile, path_length, jumps, critical_idx) {
    let path_dat = [];
    let profile_index = [];
    let dir_index = [];
    let status = [];
    ...
    for (let i = 0; i < jumps; i++) {
        if (i == 0) {
            ...
            console.log(i == before_critical);
            console.log(i == critical_idx);
            if (i == 1 && !(i == before_critical)) {
                objectUpdate = newStartPoints(profile, profile_index, dir_index, path_length);
                objectBag = combineArray(objectBag, objectUpdate);
                tmp_profile_index = checkPos(objectUpdate, startPoints)
...
function pos_only_case(startPoints, objectBag, profile, path_length, jumps, critical_idx) {
    let path_dat = [];
    let profile_index = [];
    let dir_index = [];
    let status = [];
    ...
    for (let i = 0; i < jumps; i++) {
        if (i == 0) {
            ...
            if (i == 1 && !(i == before_critical)) {
                ...
                profile_index = addArray(profile_index, tmp_profile_index);
                status = status.concat(checkStatus(tmp_profile_index));
                tmp_dir_index = pos_only_crit(tmp_profile_index);
                dir_index = addArray(dir_index, tmp_dir_index);
                tmp_path_dat = object_path(profile, tmp_profile_index, tmp_dir_index);
                path_dat = concatArr(path_dat, tmp_path_dat);
            } else {
