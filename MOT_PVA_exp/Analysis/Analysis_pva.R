...
# remove participants who did not complete all the trials
excluded_subject_pva = summary_accuracy_pva %>% filter(is.na(mean)) %>% pull(subject) %>% unique()
...
overall_summary_accuracy_pva = summary_accuracy_pva %>% group_by(trial_type) %>% summarise(N=n(),mean_overall = mean(mean),
                                                                                   sd_overall = sd(mean))
overall_summary_accuracy_pva$trial_type = factor(overall_summary_accuracy_pva$trial_type, c("position", "velocity", "acceleration"))
overall_summary_accuracy_pva = overall_summary_accuracy_pva %>% mutate(crit_speed = as.factor(rep("normal", 3))) %>% 
  select(trial_type,crit_speed,everything())
