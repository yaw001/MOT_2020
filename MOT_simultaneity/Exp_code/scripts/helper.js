...
//the critical transition for the velocity case where two objects intercept at the center including simultaneity
function velocity_crit_sim(profile_index, sim, jump_idx, pair_idx_c) {
    let len = profile_index.length;
    let dir_index = [
        [0, 0],
        [0, 0],
    ...
    if (sim == 4) {
        ...
        for (let j = 0; j < len; j++) {
            if (j == rand_index[pair_idx_c.indexOf(jump_idx)]) {
                if (profile_index[j][1] == 2 || 0) {
                    dir_index[j] = [2, 2];
                } else {
                    dir_index[j] = [3, 3];
                ...
                dir_index[j] = rand_opp_move(opp_move);
            }
        }
    }
    return dir_index;
...
function velocity_crit_sim_2(profile_index, sim, jump_idx, pair_idx_c_2, crit_1, crit_2) {
    let len = profile_index.length;
    let dir_index = [
        [0, 0],
        [0, 0],
    ...
    if (sim == 4) {
        ...
        for (let j = 0; j < len; j++) {
            if (pair_idx_c_2.indexOf(jump_idx) == 0) {
                if (checkInside(j, crit_1)) {
                    if (profile_index[j][1] == 2 || 0) {
                        dir_index[j] = [2, 2];
                    } else {
                        dir_index[j] = [3, 3];
...
//the critical condition for the acceleration case where two neighoring objects intercept at the center (including the simultaniety)
function acc_crit_sim(profile_index, sim, jump_idx, pair_idx_c) {
    let len = profile_index.length;
    let dir_index = [
        [0, 0],
        [0, 0],
        ...
        for (let j = 0; j < len; j++) {
            if (j == rand_index[pair_idx_c.indexOf(jump_idx)]) {
                ...
                if (isEqual(profile_index[j], [0, 1]) ||
                    ...
                    isEqual(profile_index[j], [2, 3]) ||
                    isEqual(profile_index[j], [3, 0])) {
                    dir_index[j] = rand_nei_move(nei_move_0);
                } else {
                    dir_index[j] = rand_nei_move(nei_move_1);
                }
            }
        }
    }
...
function acc_crit_sim_2(profile_index, sim, jump_idx, pair_idx_c_2, crit_1, crit_2) {
    let len = profile_index.length;
    let dir_index = [
        [0, 0],
        [0, 0],
